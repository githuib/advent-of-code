...
from advent_of_code.utils import debug_table, timed
...
class _AStarState(
    _State[AStarConstants, Variables], AStarState[AStarConstants, Variables]
):
    @property
...
class _Problem(NumberGridProblem[int], ABC):
    ...
    def shortest_path(self, start: str, end: str) -> int:
        ...
        p_bfs, _, t_bfs = timed(
            lambda: _BFSState.find_path(Variables(pos=start_pos), c)
        )
        visited_points_bfs: set[P2] = {s.v.pos for s in p_bfs.visited}
        p_dijkstra, _, t_dijkstra = timed(
            lambda: _DijkstraState.find_path(Variables(pos=start_pos), c)
        )
        ...
        if end_pos:
            ...
            p_a_star, _, t_a_star = timed(
                lambda: _AStarState.find_path(Variables(pos=start_pos), ac)
            )
            visited_points_a_star: set[P2] = {s.v.pos for s in p_a_star.visited}
            a_star_result = [
                (
                    (f"{chalk.hex('034').bg_hex('bdf')('E')} end", 5),
                    "A*",
        ...
        if AOC.debugging:
            ...
            log.debug(
                self.grid.to_str(
                    lambda p, _: (
                        chalk.hex("034").bg_hex("bdf")(hill_chars[p])
                        if (p in {start_pos, end_pos} | ep)
                        else chalk.hex("068").bg_hex("0af")(hill_chars[p])
                        if (p in p_points)
                        else chalk.hex("535").bg_hex("848")(hill_chars[p])
                        if (p in visited_points_a_star)
                        else chalk.hex("424").bg_hex("636")(hill_chars[p])
                        if (p in visited_points_dijkstra)
                        else chalk.hex("212").bg_hex("424")(hill_chars[p])
                        if (p in visited_points_bfs)
                        else chalk.hex("222").bg_hex("333")(hill_chars[p])
                        if (hill_chars[p] == end_val)
            ...
            debug_table(
                [
                    ...
                    ("", "BFS", len(p_bfs.visited), t_bfs),
                    (
                        (f"{chalk.hex('034').bg_hex('bdf')('S')} start", 7),
                        "Dijkstra",
                    ...
                    *a_star_result,
                    (
                        (f"{chalk.hex('068').bg_hex('0af')('p')} path", 6),
                        "",
                        "",
                        "",
                        "",
                    ...
                    (
                        (
                            f"{chalk.hex('535').bg_hex('848')('x')} visited by all algorithms (A*, Dijkstra & BFS)",
                            48,
                        ),
                        "",
                        "",
                    ...
                    (
                        (
                            f"{chalk.hex('424').bg_hex('636')('y')} only visited by Dijkstra & BFS",
                            32,
                        ),
                        "",
                        "",
                    ...
                    (
                        (
                            f"{chalk.hex('212').bg_hex('424')('z')} only visited by BFS",
                            21,
                        ),
                        "",
                        "",
                    ...
                    (
                        (
                            f"{chalk.hex('222').bg_hex('333')('a')} possible starting points (including un-escapable)",
                            51 + 68,
                        ),
                        "",
                        "",
                    ...
                    (
                        (
                            f"{chalk.hex('222').bg_hex('000')('w')} wild, unexplored terrain",
                            26,
                        ),
                        "",
                        "",
